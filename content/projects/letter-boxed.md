...
It started as a Java project for [CS 112](https://www.cs.bu.edu/courses/cs112/) in Spring 2022. A summer rewrite moved the recursive backtracking solver to [TypeScript](https://www.typescriptlang.org), added a [React](https://react.dev) frontend to visualize it, and deployed it to GitHub Pages.

The solver did all its work on the client, and heavy puzzles would freeze the UI. Fixing that took a few iterations: first the algorithm moved to a Google Cloud Function (which stopped the freezing but added network latency), then that function was rewritten in [Go](https://go.dev) to bring latency back down. Eventually it came home to the client for good — running in a [Web Worker](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API) so the heavy computation never blocks the interface.
